...
public class CassandraStore extends Store {
  ...
  @Nonnull
  @Override
  public ListenableFuture<ListenableFuture<Boolean>> renameLabel(final String newName,
                                                                 final LabelId id,
                                                                 final LabelType type) {
    // Get old name
    final ListenableFuture<List<Row>> oldNameFuture = getNameRows(id, type);

    return transform(oldNameFuture, new Function<List<Row>, ListenableFuture<Boolean>>() {
      @Override
